...
*   **Idiomatic Iroh Connection Pooling:** 
    *   **Design:** Instead of maintaining a custom `connection_cache` (with locks and state machines) in the coordinator to deduplicate connections, we use Iroh's native `Endpoint` behavior. Iroh inherently tracks active connections by `NodeId` and transparently multiplexes new requests over existing QUIC connections.
...
*   **Data at Rest & Envelope Encryption:** 
    ...
    *   **The "Unlock" Model:** DEKs are scoped to individual services from day one. The KEK starts substrate-global (M3, [ADR-0006](decisions/0006-sqlite-encryption-sqlcipher.md)) and narrows progressively. **M4 (M04A Slice B6) delivers a per-SynApp-Instance KEK *derived* from the injected master** (HKDF-SHA256, scoped by `service_id`) — a leaked derived key exposes neither the master nor a sibling instance's key, but the master itself still derives every instance's key, so this is defense-in-depth, not tenant isolation. **IAM-gated per-instance *provisioning*** — a distinct, separately-injected KEK per instance (per-service as the eventual target), so one instance's KEK grants no access to another's — is the target this narrows toward and is **not yet delivered**; per [ADR-0006](decisions/0006-sqlite-encryption-sqlcipher.md)'s Amendments, production multi-tenant-at-rest security is gated on it. Keys are *never* stored on the substrate's disk in plaintext. Once that provisioning channel ships, a service will remain locked upon node restart until its owner provisions its own KEK into RAM through an authenticated management channel, optionally after attestation; today the single injected master unlocks every service via derivation.
...
*   **Object Service (Blob Store):** 
    ...
    *   **Durability:** The durable backend is configurable S3-compatible object storage — in which case the provider is responsible for redundancy — or, in a pure peer-to-peer deployment with no S3-compatible backend configured, Syneroym's own peer-to-peer blob replication across Substrate nodes (M7, see `[PLT-RED]`). Content-addressing makes this simpler than WAL/log replication: no ordering or sequence invariants, just ensuring a verified copy of each hash exists on the configured number of peer nodes. Blob durability is never guaranteed by SQLite WAL replication, which only covers structured record state.
...
*   **P2P Overlay over QUIC (Pub/Sub):** Note that this is *not* a classical centralized TCP broker. Under the hood, publishers append to a local state log, and subscribers pull/sync these updates over Iroh QUIC. It provides the semantic decoupling of MQTT (topic wildcards, QoS, retained messages) over a decentralized peer-to-peer transport, optimized for UI updates and IoT telemetry.
*   **Protocol Bridge Design:** The Syneroym Rust host embeds the messaging capability natively as part of the core data service. It runs as a background Tokio task. External clients (via JSON-RPC or the HTTP bridge below), internal host tasks, and WASM components can all trigger publish, subscribe, or stream-protocol registration. **Interim security posture (shipped in M3B/M3C, to be closed by M4):** this native-capability dispatch path — and the HTTP bridge below, which routes through it — does **not** currently require a delegation certificate. `HandshakeVerifier::verify_preamble` only runs when `preamble.delegation` is present on the connection, and native-capability interfaces (`data-layer`, `messaging`, `blob-store`, `vault`, `app-config`, `http-native`) never require one. Any peer that can open an Iroh connection to the node and knows a target service's DID can act "as" that service on these interfaces — publish, subscribe/SSE-eavesdrop, `data-layer` writes — with no cryptographic proof of being it. This is not limited to `client_gateway`'s `127.0.0.1`-only convenience proxy: the target node's own Iroh QUIC listener (default bind `0.0.0.0`) is directly reachable by any peer. Closing this is explicit, tracked scope for M4's FDAE/UCAN work (see [meta-implementation-plan.md, Milestone 4](planning/meta-implementation-plan.md#milestone-4-typed-communication-and-authorization)), not an incidental follow-on.
*   **The WASM Boundary (WIT):** For WASM applications, interaction with the messaging system occurs via a lightweight host boundary, split into host-imported triggers, a guest-exported push surface, and a pair of stateful, **guest-implemented** resources — a pull-direction iterator for guest-produced streams and a push-direction sink for guest-consumed streams. `stream-types` is exported (not imported) by the guest world specifically because the host calls methods *on* an instance the guest returns, the reverse of `blob-store`'s host-implemented `blob-writer`/`blob-reader` — see [ADR-0014](decisions/0014-quic-stream-protocol-routing.md) "Resource Mechanics":
    ...
    ```
*   **Pub/Sub Execution Flow:** When a caller (such as a WASM component calling `host-api::publish`) triggers a publish event, the host routes it to the broker. In M3B this is a local in-process `rumqttd` Tokio task ([ADR-0010](decisions/0010-mqtt-broker-rumqttd.md)), owned by whichever node hosts the target service. A caller on a different physical node reaches it the same way it reaches any other cross-node host-function call — via the standard RPC/native-dispatch routing (JSON-RPC bridge today; wRPC once the Universal Proxy ships in M4) — exactly as a cross-node `data-layer` call is routed to wherever that service's SQLite file lives; there is nothing pub/sub-specific about this. Separately, in M7 the same `rumqttd` bridge is adapted to synchronise its topic log with peer nodes via pull-based log replication over Iroh QUIC streams rather than raw TCP, making the broker's own state durable if its hosting node is lost — sharing its replication primitive with `[PLT-RED]`'s SQLite WAL shipping (see [§ PLT-RED](#plt-red-service-redundancy)) rather than shipping in M5, since it's the same pull-based log-sync-over-QUIC problem as database replication. This is a redundancy/failover feature, not a prerequisite for cross-node pub/sub to function. Subscriptions are persisted by the host (`messaging_subscriptions` in `substrate.db`, replayed on startup). When delivering to a WASM component, it invokes the guest-exported `guest-api::handle-message` through the normal Wasmtime invocation path; a component that doesn't export it has its subscription registered but messages are silently discarded. A native (non-WASM) subscriber gets push delivery over its own live connection instead — see `SyneroymClient::subscribe` / `MessageStream` (M3B Slice 6A).
*   **Streaming Out Execution Flow — Guest as Source (M3C):** A guest calls `host-api::register-stream-protocol` to declare interest in direct, 1-to-1 streams on a protocol namespace (persisted via the existing `EndpointRegistry`, not a separate table — ADR-0014 deviation 2). When a remote peer opens a QUIC stream against that namespace requesting data, the host routes the request to `guest-api::handle-stream-request(protocol, peer-id, request-data)`. If the guest wants to fulfil it, it allocates its own internal cursor state (e.g. a file offset) and returns a `stream-cursor` resource; the host then loops `next-chunk()` in an async Tokio task, transmitting each chunk over the Iroh QUIC stream until the guest returns `Ok(None)` (EOF) or `Err` (abort), at which point the host drops the handle and closes the stream. This direction mirrors the `blob-writer`/`blob-reader` resource pair from the blob-store WIT (Slice 5), but is guest- rather than host-implemented.
*   **Streaming In Execution Flow — Guest as Sink (M3C):** The reverse direction, for a remote peer *sending* a large upload (e.g. an HTTP `PUT` bridged onto messaging, or a peer-to-peer file push). The host accepts the inbound QUIC stream and calls `guest-api::accept-stream-upload(protocol, peer-id, metadata)` to ask the guest whether it wants the data. If yes, the guest allocates its own sink state (e.g. an open file handle or a `data-layer`/`blob-store` write session) and returns a `stream-sink` resource. The host then runs an async Tokio loop reading off the QUIC stream: for each chunk that arrives, it synchronously calls `stream-sink::push-chunk(data)`, and when the QUIC stream closes, it calls `stream-sink::finalize()` so the guest can commit and release its state. A guest that declines (`Err` from `accept-stream-upload`) causes the host to close the incoming stream without ever creating a sink; `push-chunk` returning `Err` mid-upload aborts without ever calling `finalize`.
*   **HTTP Passthrough (M3C, Slice 7; extended by M06A A2):** The target node's own hyper server (`RouteHandler::handle_http_stream`, `crates/router/src/route_handler/http.rs` — not `client_gateway`, which is a byte tunnel that only sniffs the `Host:` header) bridges conventional HTTP verbs onto the same native-dispatch surface used above, per a per-service `http_routes` table declared in `ServiceConfig.custom_config`: `GET`/`POST`/`PUT`/`PATCH` map onto `data-layer` and `messaging::publish`; `GET /blobs/{hash}?svc=&exp=&sig=` resolves `blob-store`'s signed-URL scheme over a live streaming response body; `GET` with `Accept: text/event-stream` on a topic-mapped route subscribes via the same `MqttBroker::subscribe` the native push-delivery path above uses, re-emitting each message as an SSE frame; a chunked `PUT` drives the stream-protocol machinery from the previous bullet (`accept-stream-upload`/`stream-sink`) directly, with no separate blob-writer upload path. **A fourth target, `guest` (M06A A2), hands the request to the deployed component's own `syneroym:http/incoming-handler#handle-request` export** — reached by calling the sandbox engine directly, since an `http-native` connection always resolves to a `NativeService` pipeline and can never reach a guest through the JSON-RPC bridge. All bodies share one streaming-response-body type (`UnsyncBoxBody<Bytes, Infallible>`) so SSE, large blob `GET`s, and chunked uploads reuse the same infrastructure. **Gap closed (M04A Slice B0), and its scope corrected (M06A A2):** this claim previously read as though it covered the whole bridge; it never has. `data-layer`/`messaging` still require a verified caller identity before dispatch — `HandshakeVerifier::verify_preamble` is mandatory (no longer conditional on `delegation.is_some()`), an anonymous caller is rejected before the native service is invoked (bridged routes return HTTP 401), and `creator_id` is the caller's DID, not the callee service. The `stream` target has never gone through that check (it reaches guest code directly, with no caller gate at all — a pre-existing gap tracked in `deferred-backlog.md`), and `guest` adds a second, *declared* exception: a route reaches guest code with no verified caller only when it explicitly sets `public: true` (`D-A2-7`) — the default is authenticated, matching `data-layer`/`messaging`. **What `public` actually gates, precisely:** a direct anonymous connection (WebRTC, or raw QUIC presenting no usable pubkey and no delegation) — nothing reached through the local client gateway or any `SyneroymClient`, both of which always self-assert a pubkey, so `caller` is `Some` there regardless and the 401 never fires; `public: false` therefore separates "no identity at all" from "some self-asserted identity," and is not itself authentication. See `docs/planning/milestones/M04A-proxy-and-auth-foundation/status.md`'s "Slice B0" section and `docs/planning/milestones/M06A-app-platform-surface/slice-a2-implementation-plan.md` (`D-A2-7`, `D-A2-12`, F5a). `docs/planning/milestones/M03B-messaging/status.md`'s "Interim HTTP-write security posture" section records the prior interim state for history.
...
*   **Protocol Translation Architecture:**
    ...
    *   **Instance Routing:** The proxy resolves the generic WIT import to a specific deployed instance's `service_id` against the App Registry, and does so **per call rather than baking the route in at instantiation** ([ADR-0021](decisions/0021-binding-propagation-and-app-supervisor.md) §2). The developer codes against generic contracts; the Substrate handles disambiguated instance routing, and a rebinding pushed by the App Supervisor takes effect on the next call without recycling the component. A guest never holds a target DID of its own, so it cannot go stale against one.
...
*   **App Supervisor (Active Control Plane)**: A long-running component that holds an app's desired state across substrates and keeps it true — deploy-time checks and retries, health monitoring, bounded remediation, alerting, and pushing dependency bindings into constituent services. It exposes one interface, `supervisor`, consumed by `roymctl` — including an operator-facing read surface for status, alerts, and binding convergence. What it does **not** have is a service-facing directory interface: no constituent service resolves anything by calling it, and nothing queries it on the hot path ([ADR-0021](decisions/0021-binding-propagation-and-app-supervisor.md)). It runs as a substrate role rather than as a WASM `SynApp`, because it must hold delegated deploy capabilities for *other* substrates, reach them directly, and own durable storage — the same reasons the coordinator and community-registry components are substrate roles.
*   **Bootstrapping**: The supervisor is enabled by configuration on a substrate the operator controls; the user then points `roymctl` at it rather than pushing to substrates directly. Direct `roymctl`-to-substrate deployment is **permanent**, not a transitional mode: it is how the supervisor itself is stood up, and the recovery path when a supervisor is unavailable.
...
*   **Authoritative Ledger (App Supervisor)**: The supervisor's SQLite database stores Desired State (manifests, placement, current bindings) against Actual State (health polled from substrates), driving its continuous background reconciliation loop. It is designed to be **rebuildable** — from the manifests the operator holds plus a sweep of the target substrates — rather than assumed durable, because replication (Iroh WAL shipping) does not arrive until M7. Until then, losing the supervisor's node costs a rebuild, not the app.
...
*   **2. Elevated Init Execution**: The Substrate loads the new version of the WASM binary and invokes its exported `init()` (or `migrate()`) lifecycle hook. Importantly, the Substrate injects an elevated capability (e.g., an Admin UCAN) into this execution context.
*   **3. DDL and Transformation**: Within `init()`, the WASM code uses generic data-layer host functions (e.g., `execute_sql`) to perform its schema changes (DDL) and any necessary data transformations. The elevated capability allows the execution of DDL, whereas standard REST/RPC invocations are sandboxed to restricted CRUD operations.
