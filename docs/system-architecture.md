...
*   **Protocol Bridge Design:** The Syneroym Rust host embeds the messaging capability natively as part of the core data service. It runs as a background Tokio task. Any authenticated caller (e.g., external clients via JSON-RPC, internal host tasks, or WASM components) can trigger publish, subscribe, or stream-protocol registration.
*   **The WASM Boundary (WIT):** For WASM applications, interaction with the messaging system occurs via a lightweight host boundary, split into host-imported triggers, a guest-exported push surface, and a pair of stateful resources — a pull-direction iterator for guest-produced streams and a push-direction sink for guest-consumed streams:
    ```wit
    interface host-api {
        publish: func(topic: string, payload: list<u8>) -> result<_, string>;
        subscribe: func(topic: string) -> result<_, string>;
        register-stream-protocol: func(protocol: string) -> result<_, string>;
    }
    interface stream-types {
        resource stream-cursor {
            next-chunk: func() -> option<list<u8>>;
        }
    ...
    interface guest-api {
        ...
        handle-stream-request: func(peer-id: string, request-data: list<u8>) -> result<stream-cursor, string>;
        accept-stream-upload: func(peer-id: string, metadata: string) -> result<stream-sink, string>;
    }
    ```
*   **Pub/Sub Execution Flow:** When a caller (such as a WASM component calling `host-api::publish`) triggers a publish event, the host routes it to the broker. In M3B this is a local in-process `rumqttd` Tokio task ([ADR-0010](decisions/0010-mqtt-broker-rumqttd.md)), owned by whichever node hosts the target service. A caller on a different physical node reaches it the same way it reaches any other cross-node host-function call — via the standard RPC/native-dispatch routing (JSON-RPC bridge today; wRPC once the Universal Proxy ships in M4) — exactly as a cross-node `data-layer` call is routed to wherever that service's SQLite file lives; there is nothing pub/sub-specific about this. Separately, in M7 the same `rumqttd` bridge is adapted to synchronise its topic log with peer nodes via pull-based log replication over Iroh QUIC streams rather than raw TCP, making the broker's own state durable if its hosting node is lost — sharing its replication primitive with `[PLT-RED]`'s SQLite WAL shipping (see [§ PLT-RED](#plt-red-service-redundancy)) rather than shipping in M5, since it's the same pull-based log-sync-over-QUIC problem as database replication. This is a redundancy/failover feature, not a prerequisite for cross-node pub/sub to function. Subscriptions are persisted by the host. When delivering to a WASM component, it invokes the guest-exported `guest-api::handle-message` through the normal Wasmtime invocation path; a component that doesn't export it has its subscription registered but messages are silently discarded.
*   **Streaming Out Execution Flow — Guest as Source (M3C):** A guest calls `host-api::register-stream-protocol` to declare interest in direct, 1-to-1 streams on a protocol namespace. When a remote peer opens a QUIC stream against that namespace requesting data, the host routes the request to `guest-api::handle-stream-request`. If the guest wants to fulfil it, it allocates its own internal cursor state (e.g. a file offset) and returns a `stream-cursor` resource; the host then loops `next-chunk()` in an async Tokio task, transmitting each chunk over the Iroh QUIC stream until the guest returns `none` (EOF), at which point the host drops the handle and closes the stream. This direction mirrors the `blob-writer`/`blob-reader` resource pair from the blob-store WIT (Slice 5).
*   **Streaming In Execution Flow — Guest as Sink (M3C):** The reverse direction, for a remote peer *sending* a large upload (e.g. an HTTP `PUT` bridged onto messaging, or a peer-to-peer file push). The host accepts the inbound QUIC stream and calls `guest-api::accept-stream-upload(peer-id, metadata)` to ask the guest whether it wants the data. If yes, the guest allocates its own sink state (e.g. an open file handle or a `data-layer`/`blob-store` write session) and returns a `stream-sink` resource. The host then runs an async Tokio loop reading off the QUIC stream: for each chunk that arrives, it synchronously calls `stream-sink::push-chunk(data)`, and when the QUIC stream closes, it calls `stream-sink::finalize()` so the guest can commit and release its state. A guest that declines (`Err` from `accept-stream-upload`) causes the host to close the incoming stream without ever creating a sink.
