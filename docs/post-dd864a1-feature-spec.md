...
- **Unified Authorization Paradigm (Hybrid ReBAC/ABAC):** The primary authorization engine uses Casbin in a hybrid model. **ReBAC (Relationship-Based)** handles core structural relationships natively via Casbin's grouping (`g`) policies (e.g., `g(alice, editor, doc_id)`). Deeply nested organizational hierarchies (Org -> Department -> Team -> User) are managed by a dedicated Identity/Directory SynApp, which synchronizes these relationships as native Casbin `g` rules into the Substrate, ensuring evaluation is both scalable and extremely fast in-memory. **ABAC (Attribute-Based)** via the `lookup()` function is reserved purely for dynamic, stateful environmental conditions (e.g., checking if a record's status is "published").
- **Isolated Casbin Stores:** To prevent policy bleed and ensure app data remains fully portable, the authorization engines are structurally isolated. The Substrate maintains its own internal Casbin engine exclusively for management and deployment policies (e.g., node administration). Each deployed SynApp is provisioned with a dedicated, isolated Casbin store/table specifically for its own data access and execution policies.
- **Deployment Authorization:** By default, only the substrate owner (authenticated via the node's root keypair) can deploy `SynApps` to the node. The owner can delegate deployment capabilities to other identities. These deployment permissions are stored durably in the substrate's core `cr-sqlite` database. Open (permissionless) deployments are supported but require an explicit configuration flag.
- **Consent and Delegation (Bridging UCANs & Casbin):** The system supports Consent-first data and service access utilizing a hybrid delegation model. Internal system delegates use simple, stateful database-backed Casbin rules for easy revocation. For federated and external client requests, cryptographic delegation tokens (like UCANs) are used. When an external client presents a valid UCAN, the Substrate cryptographically verifies the token and translates its claims into a **temporary, request-scoped Casbin relationship** (e.g., `g(bob, alice)`). This allows Casbin to evaluate the request seamlessly without writing permanent rules to the database.
- **Identity-only Pluggable Auth:** "Pluggable Auth" specifically refers to supporting multiple Identity Providers (OIDC, DIDs, WebAuthn). These external identities all normalize into a single internal identity format, which is then evaluated against the core Casbin authorization engine. The Casbin authorization engine itself is standard and not meant to be swapped.
