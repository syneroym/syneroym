...
- [x] `query-raw` implemented, gated by Admin UCAN capability (not `is_init_context`). *(B5 — `crates/data_db/src/sqlite.rs`'s `do_query_raw` (read-only enforced via `Statement::readonly()`); guest gate in `crates/sandbox_wasm/src/host_capabilities.rs`; native gate in `crates/control_plane/src/synsvc_native.rs`'s `dispatch_data_layer`; ADR-0011 amended in place — see `status.md`'s B5 section.)*
