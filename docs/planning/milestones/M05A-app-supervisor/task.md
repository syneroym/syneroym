...
| 9 | **Supervisor finds a *higher* generation than its own** | Stops managing that instance and alerts; never self-increments (ADR-0021 §4). **✅ A5b**: `SupervisorService::handle_status` reads the max held generation across placed substrates on every sweep and raises `AlertKind::SupervisorSuperseded` when it exceeds the supervisor's own stored generation (`crates/app_supervisor/src/service.rs`) |
