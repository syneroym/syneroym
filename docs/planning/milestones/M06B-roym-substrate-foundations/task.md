...
| **B4** | **Durable messaging: interface and 1:1 delivery (G1 part 1, G2).** The `syneroym:conversation` host interface — conversations, direct delivery, delivery state, history — with outbox `pending`/`delivered`/`failed` folded in (D-06B-2). Layer 3 underneath: X3DH + Double Ratchet direct exchange, the sender's own outbox, strict direct delivery with no third-party buffering (D4). Durable content never touches the pub/sub broker (ADR-0013 §6) | **B4a Complete (2026-08-20)** — [implementation plan](slice-b4-implementation-plan.md), [status & evidence](status.md). **B4b (real X3DH + Double Ratchet, `D-B4-1`) not started** — B4a ships against a stated, non-production placeholder (`StaticEcdhSessionCrypto`), and per `D-B4-1` the slice is not complete until B4b lands and deletes it. |
