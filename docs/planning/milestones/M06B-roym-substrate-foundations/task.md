...
| D-06B-6 | **M06B builds its own throwaway fixture, not Roym.** A minimal two-party conversation fixture in `test-components/`, built both ways. **Amended by B3 (2026-08-20):** the fixture is a real **workspace member** (`test-components/dual-build-fixture/`), not excluded — a crate excluded from the workspace cannot be a dependency of `syneroym-substrate`, and "built into the substrate behind a Cargo feature" is half the definition of the slice. The *requirement* (built both ways, linked in, cheap to throw away) is unchanged; only the *mechanism* moved. | M06A's `miniapp-demo1-wasm` proved this works: a consumer cheap to throw away finds the gaps a real product would find later, at a fraction of the cost. Every gap it finds is a gap M06C would have found with four services already built on top. The exclusion clause was written before anyone tried linking a WASM-guest-shaped fixture into the substrate binary and found the contradiction; amending it in the open (matching how D-06B-4 amended M05C's S4 row) is the house rule. |
...
| B4a completes (2026-08-20) | The durable-messaging and outbox rows in [deferred-backlog.md](../../deferred-backlog.md) §5 split: the 1:1/interface/outbox half moved to "Recently resolved"; the group half (DAG, group key) stays open, retargeted at B5. New rows added for B4b (the crypto placeholder), the DLQ operator surface, and the partial cross-node e2e coverage. **Corrected from the plan's original wording**, which said "the durable-messaging and outbox rows move to Recently resolved" outright — both rows also name group delivery, which is B5's, so a straight move would have closed something not yet done. |
