...
suite (`crates/roym_web/tests/dual_build_parity.rs`, 10 scenarios). **No
exemption from D2/D3 was needed** — the entrypoint required no coarse
gating, no native-only shortcut, and no case where the two builds answer
differently beyond the permitted-differences list in
[status.md](status.md)'s C2 section.
...
| **Person sessions are in-memory** — "Empty at boot and after every restart by design" ([gateway.rs:78](../../../../crates/client_gateway/src/gateway.rs#L78)). Still true after C1.1 moves the session out of the gateway and into the auth service ([ADR-0024](../../../decisions/0024-client-gateway-identity-and-auth-service.md) §2 keeps the choice for R1) | The Hub must handle "the substrate restarted, log in again" as an ordinary state, not an error |
...
| D-06C-12 | **Every signed record is a single-issuer attestation. "Signed by both" is a completeness rule over a pair, never a second signature nested inside the first, and no signature ever carries a condition of its own.** `Envelope` has one `issuer` and one `signature` ([envelope.rs:132](../../../../crates/signed_record/src/envelope.rs#L132)) and gains neither. A record the spec marks *Signed by: Both* — `agreement-receipt`, `fulfilment-receipt` — is **two independent envelopes of the same record type**, one from each party named in the referenced `quote`, each carrying a `role` and a reference to the subject record by its derived `record_id`. **Neither half references the other.** The pair is complete when one attestation exists from each of the two named parties, the payloads are identical apart from `role`, and both fall inside their validity windows. `payment-acknowledgement` is the same shape with payload equality relaxed to the agreement reference, amount, and currency — and only there — because its two halves describe different observations (money sent, money received). **No condition field on a signature, at any version.** This row also settles the two types the spec's card table gives a *Signed by* column that `RECORD_TYPES` ([record.rs:18](../../../../crates/roym_core/src/record.rs#L18)) does not carry: `payment-request` **becomes** a signed record (attestation, issued by the provider); `booking-progress` **does not** — it is derived state, signed by the Transaction service's own DID so a reader knows it came from the single writer, and it is never evidence of anything | A countersignature — the second party signing over the first party's signature — buys nothing once the signed sentence is a content-addressed reference to the `quote`: the hash pins the terms, so two independent attestations cannot disagree about what was signed. All countersignature added was ordering, and ordering belongs to the state machine (D-06C-13), not to the signature layer. The free-form variant is worse than useless: *"I sign X subject to Y"* mints records that look mutually signed but are not, no client can evaluate the condition, and no card can render it honestly. Conditional business terms belong **inside** the `quote` both sides sign, where they are one agreed sentence with cancellation, refund, and dispute terms already beside them. Decided here rather than inside a slice for D-06C-3's reason: the shape spans C7 (`request`, `quote`, `agreement-receipt`) and C8 (payment, fulfilment), and a rule decided inside either gets decided twice, differently |
| D-06C-13 | **Payment and fulfilment are two independent tracks, and each one turns on the attestation made *against its own issuer's interest*.** Each track runs `none → claimed → acknowledged`, with `acknowledged` also reachable directly from `none`. The tracks never reference each other, so every interleaving of the four events works — payment before fulfilment and after it, either pair adjacent or split. The transaction completes when both tracks reach `acknowledged`. **The deciding half is the one that costs its issuer something**: the payee confirming receipt releases the consumer's debt, and the consumer confirming completion releases the provider. Either alone advances its track to `acknowledged`. The matching self-serving half — the payer claiming payment, the provider claiming completion — advances the track to `claimed` and no further. **The missing half is never synthesized.** The record layer keeps saying only what was actually signed; only the *outcome* stops requiring both. Each track carries its own window and ends at a named terminal — `payment_unconfirmed`, `fulfilment_unconfirmed` — holding whatever claim exists, so neither track can sit stuck. Payment timing (`before-work` / `after-work`) is a signed term of the `quote`, used to drive the "what happens next" prompt and **never** to gate a transition. UI wording follows the state exactly: `claimed` says a party *says*, `acknowledged` says the counterparty *confirms*, and no state ever says *verified* | R2's payment row requires that the UI never label an acknowledgement as verified payment, and the honest reason is that only the party who loses by speaking is worth weighing. Making the against-interest half sufficient is also what lets a flow finish when the self-serving half was never made: a consumer who never got round to recording their own payment must not block the provider's acknowledgement, and a provider who never claimed completion must not block the consumer's. Two independent tracks rather than one line is what makes pay-before-work and pay-after-work the same machine instead of two, with no ordering rule to get wrong. Ordering that genuinely matters — a losing concurrent booking — already lives in the single writer, where R2's conflict row puts it |
...
| M06B B1 — person identity at the client gateway (G3) | **Complete (2026-08-18)**. A guest HTTP handler behind an active person session sees `caller-auth = delegated` and the person's master DID ([http.wit:31-58](../../../../crates/wit_interfaces/wit/http/http.wit#L31)). **The capability survives; the mechanism is reshaped by [ADR-0024](../../../decisions/0024-client-gateway-identity-and-auth-service.md) in slice C1.1** — identity arrives as a verified `syneroym_session` cookie token instead, so `caller-auth = delegated` becomes "a valid session token, subject = the person's master DID". M06B stays Complete; B1 is not reopened |
...
| **C1.1** | **The node auth service and the dumb client gateway.** [ADR-0024](../../../decisions/0024-client-gateway-identity-and-auth-service.md), slice-breakdown item (1), planned in [slice-c1.1-implementation-plan.md](slice-c1.1-implementation-plan.md). The gateway gains an `identity_mode` (`open` / `login` / `fixed`), loses its `SessionStore` and its `/_syneroym/session/*` interception, and keeps one routing rule for auth traffic. A new `crates/auth` (`syneroym-auth`) becomes the node's identity provider: `/challenge`, `/login` (`delegated-key` and `local` only), `/methods`, `/whoami`, `/logout`, `/refresh`, and session-token minting — the person session becomes a short-lived auth-service-signed UCAN in the `syneroym_session` cookie, verified per service by one shared helper instead of arriving as a gateway-minted preamble delegation. Plus `roymctl session delegate`, `fixed` mode, and ADR-0024's fix for the gateway keep-alive bug (§P1) that C2's Hub login hits intermittently. **External identity providers (OIDC and friends) are out of scope** — C1.1 owes only the three §4 seams that keep adding one later incremental. No product code | — | C1 |
...
Sequencing): C2's login flow and C3's signing interface are both specified
against an identity model C1.1 settles, so landing either first means writing
it twice. **C2 and C3 are then independent** and can run in parallel once
C1.1 lands — one builds the app's outside, the other its record format, and
neither reads the other. C3 → C4 is then a hard line: nothing signs
...
- **What the signing interface signs under, and what it refuses.** Three
  candidate principals exist: the person's master DID (**re-grounded by
  [ADR-0024](../../../decisions/0024-client-gateway-identity-and-auth-service.md)
  / slice C1.1** — the caller is identified by a verified session token, and
  the signing key is a per-person delegation the substrate holds, not the
  gateway-minted B1 delegation), the service's derived instance key
...
- **Which service owns the person→conversation-address mapping, and how a
  ...
  **Answered on the product side by C4 (2026-09-01):** the mapping is
  `syneroym_roym_core::person::ProfilePayload::conversation_address`, stored
  by `profile` and signed into a `profile` record (the tenth record type),
  so a stranger who verifies the record has an address they can attribute
...
2. The UI is served by the Web entrypoint from **one origin**, and every
   ...
   UI involved.
3. A person logs in locally; Roym's services see **that person's** DID.
   After C1.1 that means a valid session token whose subject is the person's
   master DID
   ([ADR-0024](../../../decisions/0024-client-gateway-identity-and-auth-service.md) §3),
   not B1's `caller-auth = delegated` preamble signal. A second local process
   cannot obtain it.
...
| Document | Edit |
|---|---|
| [roym-integrated-experience-spec.md](../../../roym-integrated-experience-spec.md) | The **Records** section given a note explaining what *Signed by: Both* means (D-06C-12): a pair of independent attestations, never a nested or conditional signature; and, for `payment-acknowledgement` and `fulfilment-receipt`, that the against-interest half alone finishes the step (D-06C-13). The same note places `payment-request` in the record list and keeps `booking-progress` out of it, closing the mismatch between the card table's *Signed by* column and `RECORD_TYPES` |
...
| C1.1 completes | [ADR-0024](../../../decisions/0024-client-gateway-identity-and-auth-service.md) moved `Proposed` → `Accepted`, with a dated implementation amendment for anything that shipped differently. The developer guide's "Reserved Endpoints (`/_syneroym/session/*`)" section rewritten for the auth service — it documents the gateway-owned B1 endpoints today and goes stale the moment C1.1 merges. The §7 browser-login backlog row moved to "Recently resolved" |
