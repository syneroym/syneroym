...
> proposed below is written with that already applied. §16 item 14 checks
...
| **D-C6-9** | **The fan-out is sequential, with a per-directory deadline and a total budget, and it is sequential on both builds. "Parallel" is corrected in the spec rather than faked.** Each source gets `call-options.timeout-ms`; a source that fails, times out, or answers with an error contributes a `source_error` entry and never removes a result another source returned. | `F6`: a WASM guest cannot await two host calls at once — `block_on` panics on `Poll::Pending`. Making only the native build concurrent would produce different orderings and different partial results between builds, which failure-matrix row 19 calls a bug in the shim. The person-visible property the spec actually wants is *"one slow directory does not stop the others"*, and a deadline per source delivers that; genuine concurrency is a backlog row with a real trigger (guest async, or a node-side fan-out service). Saying it plainly in the spec is the house rule `D-06C-2` set. |
...
| **D-C6-11** | **Ordering is recency plus a deterministic tiebreak, and the product says it is not ranking.** Results sort by `issued_at_secs` descending, then `listing_id` ascending. No score, no relevance weight, no boost. | R1 row 5's Excluded column names *"Ranking; paid placement; free-text intent parsing"* explicitly. Anything that looks like a score would be the first of those three arriving by accident. `F4` also means there is no index to rank against cheaply, so a score would cost a full pass for a property the release does not want. |
...
| **D-C6-18** | **Refused results never compete with verified ones for a page.** A merged search answers `hits[]` (verified, sorted, capped at `MAX_SEARCH_RESULTS`) and `refused[]` (failed verification, capped separately at `MAX_REFUSED_RESULTS`), and the second is always rendered. Paging and ordering apply to `hits[]` alone. | The alternative — one list, failures counted toward the page — is **exploitable**. Results sort by `issued_at_secs` descending (`D-C6-11`); `verify_json` refuses a future timestamp (`VerifyError::IssuedInFuture`) but nothing stops a forger stamping *now*, so forged rows sort first. A hostile directory returning a full page of forgeries would then push every honest result from every other directory off the page, invisibly, because the person sees a full page either way. Two lists satisfy the requirement that made the single list tempting — *the person must learn a directory served forgeries* — while removing the ability to bury honest answers with it. |
| **D-C6-19** | **The fan-out's three bounds are chosen together, sources are asked in a deterministic rotating order, and a source the budget never reached says exactly that.** `MAX_SOURCES = 8`, `DEFAULT_SOURCE_TIMEOUT_MS = 3_000`, `MAX_TOTAL_SEARCH_MS = 20_000`. Sources are ordered by DID ascending and started at an offset taken from a stored per-node search counter. A source not started before the budget expires contributes `SourceError::NotReached`, never `TimedOut`. | A per-source deadline is a *failure* deadline — a healthy directory answers in well under a second — so pricing it as if it were the expected latency is what made an earlier draft's numbers inconsistent (sixteen sources at five seconds could never all be asked under a twenty-second budget, making the source cap decorative). Fixed order alone starves the last source in the list on every slow day; a rotating offset spreads that, and a **counter** rather than a clock keeps the order reproducible across builds — the parity suite drives both stacks through the same scenarios, so the counter matches, which a wall-clock or a `last_ok_secs` ordering would not. Distinguishing `NotReached` is what stops the product blaming a directory for this node's own budget. |
| **D-C6-20** | **The two halves get two method names. `directory.search` is the server half — wire-reachable, answers from this node's own SynOrg. `directory.find` is the client half — local-only, fans out over this person's sources and merges.** No method changes meaning based on who called it. | `admit`'s exception table maps *method name* → wire rule. One shared name would make `("directory.search", Open)` declare the rule for the server half while the client half rode the same string, so a later mistake widening the client half would pass the guard test unnoticed. Two names also make a subtle product rule visible instead of hidden: a node that runs a SynOrg **and** holds sources does not search itself unless its owner adds itself as a source. And the test suite reads honestly — a wire scenario and a zero-sources scenario are then two methods, not two paths behind one. `find` is not invented for this: *"Finding a provider"* is the spec's own journey heading, and the client half calling `directory.search` on a **foreign** node is the spec's word used exactly as the spec uses it. Cost is one row in the routing table. |
...
`require_internal` stays exactly as C5 shipped it and keeps its five
current callers. C6 adds a second entry point beside it.
...
pub const MAX_SEARCH_RESULTS: u32 = 50;        // verified hits, per source, per call
/// Refused hits are carried separately and capped separately, so a
/// directory serving forgeries cannot crowd out honest answers
/// (`D-C6-18`).
pub const MAX_REFUSED_RESULTS: u32 = 20;
/// Eight sources at a three-second deadline is 24 s, which the total
/// budget clips. The three numbers are chosen together and must stay
/// consistent: a per-source deadline is a *failure* deadline, and a
/// healthy directory answers in well under a second.
pub const MAX_SOURCES: usize = 8;
pub const DEFAULT_SOURCE_TIMEOUT_MS: u32 = 3_000;
pub const MAX_TOTAL_SEARCH_MS: u32 = 20_000;
...
/// Why a source contributed nothing. `NotReached` is deliberately not
/// folded into `TimedOut` (`D-C6-19`): "this directory is broken" and
/// "we ran out of budget before asking it" are different sentences to
/// show a person, and only one of them is the directory's fault.
...
### 4.5 `src/backup.rs` — two new section names

`directory_publications` and `directory_sources`, for `D-C5-21`'s rule
(*every service that owns durable product state exports its own bundle*).
Settings and members ride the first bundle's manifest as their own
sections; `search_index` is **not** exported, because it is derived
(`D-C6-3`) and `directory.reindex` rebuilds it after an import.
...
| `search_index` | `<listing_id>#<area_index>` | The projection (`D-C6-4`). Derived; droppable; rebuilt by `directory.reindex`. |
...
- `categories` is the delimited token string `F5`'s `$regex` matches
  exactly (`"|plumbing|"`), with `%`, `_` and `|` escaped out of the
  tokens at write time — validated already by `ListingPayload`'s
  `[a-z0-9-]` category shape, so the escape is a belt, not a brace.
- `text` is lowercased `title + " " + summary + " " + categories`, so one
  `$regex` covers free text. This is substring matching, not tokenized
  search, and the Hub's help text says so — *"free-text intent parsing"*
  is excluded by R1's own scope row anyway.
...
| `directory.unpublish` | local | The SynOrg owner removing a publication under its own rules (journey step S9). Removes the `publications` row and its `search_index` rows. Never touches the provider's own copy and never claims to. |
...
1. `admit::admit` has already returned `Caller::Verified(did)`
   (`D-C6-7`); the DID becomes `published_by`.
2. `listing::verify_envelope(&envelope, now)`. A verdict that is not
   ...
   requirement.
3. The payload's `conversation_address` must be non-empty. It always is
   (it is required in the signed payload since C5), and checking it here
   is what guarantees the consumer can act on a search result with no
   further lookup — the whole point of `D-C5-8`.
4. **Withdrawal path.** If `payload.status == withdrawn`, replace or
   delete the stored publication for that `listing_id`, drop its
   `search_index` rows, consume **no** budget (`D-C6-14`), and return.
5. **The limiter.** `publication_log` rows for this issuer inside the
   window → `safety::admit_publication`. `Admission::RateLimited` is
   refused with a `retry_after_secs` the caller can act on. In the same
   pass, `delete-many` removes this issuer's rows older than the window
   (`F12`).
6. Store the envelope verbatim in `publications`, keyed by `record_id`,
   with `received_at_secs = clock::now_secs()`. A newer version of the
   same `listing_id` replaces the older row rather than accumulating — a
   directory holds *the current offer*, and `listing.history` on the
   provider's own catalog is where versions live.
7. Write the `search_index` rows.
8. Append the `publication_log` row.
...
4. Collapse to one hit per `listing_id`, keeping the row whose
   ...
   `NoAreaStated`.
5. Sort by `(issued_at_secs desc, listing_id asc)` (`D-C6-11`) and take
...
Separate collections, separate prefixes, same service (`D-C6-2`).

| Collection | Key | Holds |
|---|---|---|
| `sources` | directory DID | `{ did, label, added_at_secs, last_ok_secs, last_error }`. Capped at `MAX_SOURCES`. |
| `state` | `"search_counter"` (single row) | The rotating fan-out offset (`D-C6-19`). Compared across builds (§13). |

Two collections, and no third: C6 caches no search result (§17).

| Method | Wire | Does |
|---|---|---|
| `directory.add-source` | local | Adds a directory DID by hand (`D-C6-16`). Probes it once with `directory.info` and stores the returned name as the label, so the person sees what they added. The source is added either way, and the probe's outcome is reported: a failure is stored as `last_error`, and a probe that succeeds but returns `null` is reported as *"this address answered, but runs no directory"* (`D-C6-21`) rather than stored as a silent no-op. |
| `directory.remove-source` | local | Removes it. |
| `directory.sources` | local | Lists them, with `last_ok_secs` and `last_error`. |
| `directory.find` | local | The fan-out, §6.1–6.3. **Local-only**, and a different method from the server half's `directory.search` (`D-C6-20`). |

**Two names, and no method that changes meaning based on its caller.**
`directory.search` answers from *this* node's SynOrg and is reachable from
off the node; `directory.find` asks *other* people's directories and is
reachable only from inside. The rule this makes visible: a node running a
SynOrg does **not** include itself in its own `find` unless its owner adds
itself as a source. `D-C6-20` has the reasoning.

### 6.1 The fan-out (`D-C6-9`, `D-C6-19`)

```
directory.find (internal only)
  ├─ read `sources`, order by DID ascending
  ├─ rotate the start index by (stored search counter mod n), then bump it
  ├─ for each source, in that order:
  │     proxy.call(Service(<did>), DIRECTORY.interface, "invoke",
  │                {"method":"directory.search","params":{…}},
  │                { timeout_ms: DEFAULT_SOURCE_TIMEOUT_MS, idempotent: true })
  │     ok    -> parse hits, record last_ok_secs
  │     error -> `source_errors[]` entry { directory, error }, record last_error
  │     once MAX_TOTAL_SEARCH_MS is spent, start no more:
  │             every remaining source gets SourceError::NotReached
  ├─ verify every hit locally (§6.2)
  ├─ merge the verified ones; carry the refused ones separately (§6.3)
  └─ answer { hits[], refused[], source_errors[], truncated }
```
...
**The rotation is what stops the last source in the list being starved on
every slow day**, and the counter rather than a clock is what keeps the
order reproducible across builds — a `last_ok_secs` ordering would differ
between the two stacks, because guest wall clocks are not synchronized
(permitted difference 7).

A source that fails contributes an error entry and removes nothing. **A
search with zero sources succeeds and returns zero hits**, which is the
shape `D-06C-6a` needs: a person with no directory gets an empty answer,
never an error, and every other R1 path is untouched.

### 6.2 Verification, on the consumer's own node (`D-06C-6c`)

For every hit, in the guest, `listing::verify_envelope(&hit.envelope,
clock::now_secs())`. The result carries, per hit:
...
A hit that fails verification is **kept in the answer, in its own list**
(`D-C6-18`). It is never dropped — the person asked which directories said
what, and silently deleting a forged answer hides that a directory served
one — and it is never in `hits[]`, because a forger controls
`issued_at_secs` up to now and would otherwise sort to the top of the
page and push honest results off it. The Hub renders `refused[]` as its
own block, as refused evidence, never as a result (failure-matrix row 1).

### 6.3 Merge (`D-C6-10`, `D-C6-18`)

**Verified hits.** Group by `listing_id`; keep the envelope with the
greatest signed `issued_at_secs`, ties by `record_id` ascending; union the
`sources[]`; set `versions_differ` when the kept `record_id` is not the
only one seen. Never take a field from one envelope and a field from
another. Sort by `D-C6-11`'s rule and cap at `MAX_SEARCH_RESULTS`.
...
controls — and capped at `MAX_REFUSED_RESULTS`. `truncated` is reported
per list, so a person can tell a truncated result page from a truncated
refusal list.

**The two caps are independent, and that is the whole point.** A source
returning nothing but forgeries consumes none of the space honest sources
compete for. Parity has a scenario for exactly that (§11.2, 102b).
...
- **New: `roym directory`**, a small group that drives the same JSON-RPC
  ...
  *"a second client drives the same flow through the same API with no UI
  ...
  - `roym directory find [--text] [--category …] [--near lat,lon,radius_m] [--limit]`
    — drives `directory.find` (`D-C6-20`). Prints one line per result
    with issuer, age, source, and the verification verdict **including
    the two unknowns**, because a CLI that prints only the good news is
    the same failure the UI rule is about. Prints the `refused[]` block
    under its own heading (`D-C6-18`) and the `source_errors[]` block
    under a third, with `not reached` worded differently from `timed
    out` (`D-C6-19`).
...
| 97 | `directory.find` over two sources merges into one hit per `listing_id` with two `sources[]` entries | the two-entry array |
...
| 99 | A source that answers `ServiceNotFound` contributes a `source_errors[]` entry and removes no result from the other source | both arrays in one answer |
| 100 | A source that times out does the same, and `last_error` is stored on that source row | the stored `last_error` |
| 100b | **`NotReached` is not `TimedOut`**: with the total budget exhausted, an unstarted source reports `NotReached` and its `last_error` is left untouched — this node's budget is not that directory's fault (`D-C6-19`) | the error arm, and the unchanged `last_error` |
| 100c | **The rotation works and is reproducible**: two consecutive `directory.find` calls start at different sources, in the same order on both builds | the contact order of both calls, compared across builds |
| 101 | **`directory.find` with zero sources succeeds with zero hits and no error** — `D-06C-6a` at verb level | `hits: []`, no `error` key |
...
| 102b | **A source returning nothing but forgeries reduces `hits[]` by nothing.** Source A returns `MAX_SEARCH_RESULTS` forged listings stamped `now`; source B returns ten good ones; all ten good ones are in `hits[]` and every forged one is in `refused[]` (`D-C6-18`) | `hits.len() == 10`, and the identical answer with source A absent |
| 102c | `hits[]` and `refused[]` are capped and truncated independently | two `truncated` flags with different values in one answer |
...
| 106b | **`directory.find` is refused over the wire** while `directory.search` is not — the two halves have two names and two rules (`D-C6-20`) | `-32013` for one and hits for the other, in one scenario |
...
| 7 | 7 | X adds Z as a source and runs `directory.find` by category and area. Asserts: one hit; the envelope byte-identical to Y's; `sources[0].directory == Z`; `age_secs` computed on X's clock; `revocation_status == "unknown"`; `credential == "unknown"`. |
...
| 18 | A source that errored shows its error beside it and the other sources' results still render — and a source the budget never reached says *"not reached"*, worded differently from *"timed out"* (`D-C6-19`). |
...
| **19** (build divergence) | 41 new parity scenarios, of which 79/80/94/102b/106b/107 are the ones that matter: the new wire arms behave identically on both builds, in both directions. |
...
10. `sources`, the fan-out, the verification pass, the merge (§6).
...
14. The four parity harness changes, then scenarios 74–108 (§11.2).
...
17. The Hub's Directory and SynOrg tabs, its vitest additions, and
    `roym-hub.spec.ts` cases 13–20 (§11.4) — case 20's publication-limit
    control is required, not optional (`D-C6-22`). Rebuild with
...
- **Compared as an ordered list, not as a set:** `hits[]` and
  ...
  one thing most likely to break. Also the **order sources were
  contacted in**, which `D-C6-19`'s rotation makes reproducible.
- **Compared, and easy to forget:** the `state` collection's
  `search_counter`. It is the input to the rotation, so if it diverges the
  contact order diverges with it, and scenario 100c fails for a reason
  that looks like a fan-out bug and is not.
...
13. **The fan-out is sequential on both builds, by decision, not by
    limitation** (`D-C6-9`). The native shim *could* drive concurrent
    proxy calls; it deliberately does not, because the WASM build cannot
    (`F6`) and a difference in ordering or in which partial results
    survive a deadline would be exactly what failure-matrix row 19 calls a
    bug in the shim. **Not permitted:** any concurrency added to the
    native path without the same being possible in WASM.
14. **The wire origin still has no production producer on the native
...
15. **Guest-HTTP and websocket sinks now report the same origin on both
...
9. A slow or dead directory produces an error beside its own name and
   removes nothing another directory returned; a directory the budget
   never reached says so in different words from one that timed out; and
   the same source is not starved on every call (`D-C6-19`).
10. **The whole R1 find-and-engage path completes with no directory
    anywhere** — proven twice in the e2e, once before any directory
    exists and once after one has been added and removed. `D-06C-6a`.
11. `directory.search`, `directory.info` and `directory.publish` are
    reachable from off the node — and `directory.find`, the client half,
    is **not**, because it is a different method (`D-C6-20`);
    `directory.publish` refuses an
    anonymous caller; **every other method on every Roym service still
    answers `-32013` over the wire on both builds**, proven by parity 107
    in both directions.
12. `safety::admit_publication` has its second caller, keyed on the
...
13. `CallTarget::Service` appears in no Roym crate but `roym_directory`,
    asserted by a test rather than by review (§7.1, parity 108).
14. All 41 new parity scenarios pass identically on both builds, and the
    two-substrate suites grow to a three-substrate one that passes all
    fourteen steps.
15. `cargo xtask check-roym-deps` is clean, and a grep over
...
16. The full gate in §12 step 18 is clean.
17. §15's documents and backlog rows are written — including the two
...
- **Concurrency in the fan-out.** `F6`, `D-C6-9`.
...
**B. The spec's "in parallel" is not implementable in a guest.**
`block_on` panics on `Poll::Pending` and states the invariant in its own
comment (`F6`). The property the spec actually wants — one slow directory
not stopping the others — is delivered by a per-source deadline.
`D-C6-9` and §15's spec edit; recorded rather than quietly approximated.
...
method is `directory.search` for the server half and `directory.find` for the client half (`D-C6-20`). This is the same "the table is a summary,
...
## §19 Questions closed before execution, and what remains open

All seven questions an earlier draft left to the executor were decided on
review (2026-09-04). They are recorded here with their outcomes rather
than deleted, because two of them were **defects** and the reasoning
should not have to be rediscovered.

| # | Question | Outcome |
|---|---|---|
| 1 | Does the native HTTP/websocket rewiring belong in C6? | **Kept, restructured.** `D-C6-17` — its own severable work order (WO5), tested in the *shim's* suite against the dual-build fixture, not in Roym's. "Cheap and thematically related" was not a sufficient reason; being verifiable now is. |
| 2 | One `directory.search`, or two names? | **Changed to two.** `D-C6-20` — `directory.search` (server, wire) and `directory.find` (client, local). One name would have let the wire-rule table cover the client half by accident. |
| 3 | Publish on `catalog` or on `directory`? | **Unchanged, reason strengthened.** §7.1 — `directory` becomes the only Roym service that talks to a foreign node, and parity 108 asserts it instead of leaving it a convention. |
| 4 | Is 20 publications per 24 hours the right default? | **Default unchanged; the control is now required.** `D-C6-22` — the limit is the group's policy, so the owner must be able to set it from the Hub (browser case 20). A default nobody can find was the real problem. |
| 5 | `MAX_SOURCES` and the deadlines | **Corrected — the numbers were inconsistent.** `D-C6-19` — sixteen sources at five seconds could never all be asked inside a twenty-second budget, making the cap decorative. Now 8 / 3 s / 20 s, with `NotReached` distinguished from `TimedOut` and a reproducible rotating order. |
| 6 | Do failed verifications count toward the page? | **Changed — the original answer was exploitable.** `D-C6-18` — results sort by `issued_at_secs` and a forger controls that value up to now, so one list would let a hostile directory bury every honest result. Two lists, capped independently. |
| 7 | `directory.info` before a SynOrg exists | **Unchanged, with a paired half.** `D-C6-21` — `null` rather than an error, and `add-source` reports that answer instead of storing a silent no-op. |

### Still open, and genuinely the executor's

1. **How the Hub renders a partial answer while the fan-out is still
   running.** `D-C6-9` makes the fan-out sequential and
   `MAX_TOTAL_SEARCH_MS` makes a slow set of sources take real time, so a
   single spinner over twenty seconds is the wrong shape. Progressive
   rendering per source is the obvious answer and needs no server change
   — `directory.find` could instead be called once per source from the
   browser, with the merge done in the page. The plan does **not** take
   that option, because exit criterion 2 requires a second client to
   drive the same flow with no UI, which means the merge has to exist on
   the node anyway. If the Hub also merges, the two must agree, and a
   test must say so.
2. **Whether `MAX_REFUSED_RESULTS = 20` is too low to be honest.** A
   directory serving 200 forgeries shows 20 and a truncation flag. That
   is arguably enough to make the point, and arguably hides the scale of
   it. A count of what was refused, alongside the capped list, is the
   cheap middle answer and is not in the plan.
3. **Whether `directory.unpublish` should tell the provider.** The SynOrg
   owner removing a listing under their own rules (journey step S9) is
   silent in this plan: the provider learns only by searching. A
   notification would need the provider's conversation address, which the
   listing carries — so it is buildable, and it is deliberately not built,
