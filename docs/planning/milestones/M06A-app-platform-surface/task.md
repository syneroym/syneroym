...
| **A5** | **The Playwright suite.** The four cases from `webrtc.spec.ts` re-pointed at the demo app — including echo/broadcast over a real WebSocket, matching the native fixture — plus a fifth covering the SSE path | **Complete** |
