...
| **G3** | No undeploy cleanup for `websocket_senders` | **Accepted.** Added `forget_websocket_senders`. Changed map signature to group by `service_id` so dropping the map cleanly aborts all active `rx.recv()` writer loops, which in turn send a `session_stop` signal to break the reader loop and close the socket. `D-A3-7`. |
...
| **D-A3-7** | **Undeploy cleanup drops sender maps.** `forget_websocket_senders` removes the `service_id` key from the registry. | Dropping the map drops the `Sender`s for that service. This safely causes the `rx.recv()` in the router's connection writer loops to return `None`, which then signals `session_stop` to break the reader loop and close the underlying socket without hanging tasks. |
