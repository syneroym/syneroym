...
| Cross-node `expected_asserter_did` discovery/publication | B3's `resolve_fetches` cryptographically verifies a remote fetch's `RelationshipProof` against a policy-declared `expected_asserter_did` (D-B3-8), but that DID is derived from the *remote* node's own private key — a policy author on the requesting node has no way to learn it without an out-of-band channel; no publication/lookup mechanism exists yet. Every passing B3 end-to-end test only works because both services share one in-process node identity. Genuine cross-node fetch — B3's actual purpose — is not yet operable end-to-end. | TBD | [M04B slice-b3 plan](./milestones/M04B-fdae-policy/slice-b3-implementation-plan.md) D-B3-8 residual gap |
