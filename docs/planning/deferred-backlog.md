...
| D-04-02-h guest-originated-read identity threading | Neither ingress (direct WASM host-function read, nor guest self-proxy) carries a real external caller into `HostState`/native dispatch -- both synthesize `service_system`, since `router/src/route_handler/dispatch.rs`'s `JsonRpcToWasm` branch drops the verified caller before `execute_wasm_json`. Confirmed in B3 Phase 4 that the two ingresses share this one root cause and can't be closed independently. **Update (Phase 5, 2026-07-25):** given its own scheduled slice rather than left open-ended — deferred three consecutive phases (B2 Phase 4, B3 Phase 4, B3 Phase 5) is a pattern, not a coincidence, and it blocks a substantive claim (reference scenario step 22, the primary SynApp usage pattern of a WASM guest reading its own data), not a formality. | Slice B3.5-fdae | [M04B task](./milestones/M04B-fdae-policy/task.md) D-04-02-h, Slice B3.5-fdae; [M04B slice-b3 plan](./milestones/M04B-fdae-policy/slice-b3-implementation-plan.md) §7 D-B3-4 |
