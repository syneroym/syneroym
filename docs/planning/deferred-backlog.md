...
| Federated fetch result caching/TTL (D-B3-6) | No caching of a fetched `RelationshipProof` across calls — every `resolve_fetches` call re-fetches from the remote node, even for the same `(service, relation, principal)` within one request's evaluation. ADR-0017 §6 calls batching + TTL'd caching "load-bearing … or Mode B does not scale," so this is a real scale gap, not a nice-to-have — shipped without it deliberately (correctness first), *provided* the signed, TTL'd `RelationshipProof` record shape (already shipped, §3.2) lands now so a cache is a pure additive follow-up with no wire-format churn later. Distinct from the parallelism item above (that's concurrency *within* one request; this is reuse *across* requests/time). | TBD (perf, scale) | `crates/rpc/src/fdae_fetch.rs`; `crates/rpc/src/relationship_proof.rs`; [M04B slice-b3 plan](./milestones/M04B-fdae-policy/slice-b3-implementation-plan.md) §3.2/§5/§7 D-B3-6; [ADR-0017](../decisions/0017-fdae-policy-schema-and-compilation.md) §6 |
| Cross-node `expected_asserter_did` discovery/publication | B3's `resolve_fetches` cryptographically verifies a remote fetch's `RelationshipProof` against a policy-declared `expected_asserter_did` (D-B3-8), but that DID is derived from the *remote* node's own private key — a policy author on the requesting node has no automated way to learn it, only an out-of-band channel (the deploying operator communicates it directly, the same category of explicit trust knob as `resolvable_without_capability`). **Update (Phase 5, 2026-07-25):** genuine cross-node operability itself is no longer in doubt — `crates/substrate/tests/federated_fdae_e2e.rs` proves the fetch working end-to-end across two independently-identified `syneroym-substrate` instances (distinct node identities, distinct storage, a real Iroh QUIC hop), with the DID learned exactly as a real deployment would: read directly off the data-owning node by whoever provisions it. What remains deferred is only the *automated* discovery/publication mechanism (a lookup service, so a policy author never needs the out-of-band step at all) — not "does this work across nodes," which is now demonstrated. | TBD | [M04B slice-b3 plan](./milestones/M04B-fdae-policy/slice-b3-implementation-plan.md) D-B3-8 residual gap |
| `IrohHop` connection reuse for federated fetches | `resolve_fetches`'s each remote hop opens a brand-new Iroh QUIC connection (`crates/router/src/proxy.rs`'s `IrohHop::call`) rather than reusing a warm one to the same peer. Discovered while measuring Phase 5's federated-fetch perf budget: on the sandboxed local test environment, connection establishment alone consistently dominated the measured latency (~4.5s observed, vs. the < 50 ms p99 aspirational floor), with zero retries logged — i.e. the cost is inherent to a fresh connection, not a symptom of anything else. Distinct from, and more fundamental than, the already-recorded fetch-parallelism item above. | TBD (perf) | `crates/router/src/proxy.rs`; [M04B slice-b3 plan](./milestones/M04B-fdae-policy/slice-b3-implementation-plan.md) §9; `crates/substrate/tests/federated_fdae_e2e.rs` |
