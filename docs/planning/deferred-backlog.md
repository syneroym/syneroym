...
| Stage-4 ABAC widens `aggregate`/`delete_many` denial | `aggregate`/`delete_many` now deny closed outright under a stage-4-opted policy (rows never surface to filter after the fact) — a new, real, user-visible restriction (a permission that used to work under CLS-only masking now hard-denies once `authorize_rows: true` is added), not merely a defensible extension of the existing CLS precedent at `sqlite.rs:636`. No mechanism exists to run the after-step over an aggregation pipeline's grouped/computed output or over rows about to be bulk-deleted before they're gone. | TBD | [M04B slice-b4 plan](./milestones/M04B-fdae-policy/slice-b4-implementation-plan.md) §3.7/D-B4-3; `crates/data_db/src/sqlite.rs` (`do_aggregate`, `do_delete_many`) |
...
| Self-proxy write attribution disagrees with the direct-WIT write path | Slice B3.5-fdae's identity forwarding made `SynSvcNativeService`'s `put`/`batch-mutate` (`crates/control_plane/src/synsvc_native.rs`) reachable via a guest's self-proxy with the real caller in `invocation.caller`, so `creator_id` now stamps the real caller's DID on that path — while the guest's own direct WIT `put` (`crates/sandbox_wasm/src/host_capabilities.rs`) always stamps the service's own `component_id`, unaffected. Same guest, two write paths, two different attributions. `NativeInvocation` carries no origin marker to distinguish "genuinely native external caller" (where real-caller attribution is correct and intended) from "guest self-proxy" at that handler, so a real fix needs a deliberate design call, not a one-line change — folded into B5-fdae's write-path work (D-04-02-f) rather than decided here. Pinned explicitly (not silently regressed) by `router/tests/proxy_dispatch.rs::guest_self_proxy_put_attributes_creator_id_to_the_real_caller_not_the_service`. | M04B follow-on (B5-fdae) | [M04B task](./milestones/M04B-fdae-policy/task.md) (D-04-02-f, Slice B3.5-fdae); `crates/control_plane/src/synsvc_native.rs`; `crates/sandbox_wasm/src/host_capabilities.rs` |
