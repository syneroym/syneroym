...
- **First implementation addresses a service, not a person (M06B B4, 2026-08-20):** `syneroym:conversation@0.1.0` addresses a peer by its routing *service id* (`peer-address`/`author`), not by a Master DID — the transport-verified caller identity on any inbound call is the owner's Master DID, which cannot distinguish two services one owner runs, so it cannot carry attribution on its own. Decision 2's Primary Substrate designation and person→substrate resolution are unbuilt; `syneroym:conversation` is the eventual home for that resolution once it exists, but does not implement it yet.
...
- **Participant Relays:** Online members store the message. When an offline member comes online, they pull missed messages from any currently online peer in the group. No external storage is used; the group sustains its own data availability.
...
- **Total Ordering:** The chat sequence is deterministically sorted globally by `(Sender_Timestamp, Sender_DID)`. Because every peer sorts on the same immutable, signed value regardless of how many relay hops the message took to arrive, ordering is consistent across all participants by design — clock skew affects chronological *accuracy* (a message may sort earlier/later than it was "really" sent in wall-clock terms), never cross-peer ordering *consistency*. We explicitly avoid reliance on external NTP servers to maintain offline-first robustness.
  > **Implementation note (M06B B4, 2026-08-20):** `(Sender_Timestamp, Sender_DID)` is not a total order — one sender can produce two messages in the same millisecond. `syneroym:conversation`'s own store sorts `(sender-timestamp, author, message-id)`, the message's own content-derived id breaking the tie deterministically at no extra cost. B5's DAG must sort on the same three-part key or transcripts will not be byte-identical.
...
- ~~**Raw-timestamp plausibility bounds:**~~ **Resolved by M06B B4 (2026-08-20).** A message whose `sender-timestamp` is more than 24 hours *ahead* of the receiving substrate's own clock is refused on arrival. Deliberately asymmetric: a future timestamp is an ordering attack (it would pin the message to the top of every participant's history permanently, since ordering sorts ascending), while a past timestamp only sorts old and is accepted unbounded. The bound is a configured value (`conversation_max_clock_skew_secs`, `crates/core/src/config.rs`), not a hardcoded constant, so a deployment can widen it if 24 hours proves too tight for a real clock-skew population.
