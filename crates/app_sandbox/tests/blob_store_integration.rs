...
//! `HostBlobReader` WIT wiring in `crates/app_sandbox/src/engine.rs`
...
//! `open-download`/`read`), not just the `crates/blob-store` crate in
//! isolation.

use std::sync::Arc;

use syneroym_app_sandbox::HostState;
use syneroym_bindings::host::syneroym::blob_store::blob_store::{
...
use syneroym_data_layer::{SqliteStorageProvider, StorageProvider};
...
async fn test_cross_service_blob_isolation() {
    ...
    // crates/blob-store/src/object_store_impl.rs's
