...
async fn test_deployed_policy_yields_empty_guest_originated_query_d04_02_h() {
    ...
    assert_eq!(
        ...
        "a guest-originated read under a loaded policy must be empty for an unauthenticated \
         ...
         'WASM guests admit anonymous callers') -- `run_crud_scenario` drives `execute_wasm`, \
         which always synthesizes `service_system` (`caller: None`), so the write side (ungated) \
         still creates all 100 records, but the guest's own query, running as service_system, can \
         reach none of them. Slice B3.5-fdae closed D-04-02-h for a *real*caller -- see \
