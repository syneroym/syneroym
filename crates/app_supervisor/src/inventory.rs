//! The wire shape of `submission.inventory-json`: alias -> `{did, api-url,
//! ucan}`. Distinct from `syneroym_app_orchestration::
...
pub struct SupervisorInventoryEntry {
    ...
    /// The supervisor's own credential against this substrate: nothing
    /// issues it automatically, so `submit` refuses an alias that carries
    /// none.
    #[serde(default, skip_serializing_if = "Option::is_none")]
