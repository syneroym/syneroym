...
//! Native service wiring for directory service.
...
pub struct NativeDirectory<H: AppHost + 'static> {
    service_id: String,
    host_for: Box<dyn Fn(CallerContext) -> H + Send + Sync>,
}

impl<H: AppHost + 'static> fmt::Debug for NativeDirectory<H> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NativeDirectory")
            .field("service_id", &self.service_id)
            .finish_non_exhaustive()
    }
}

impl<H: AppHost + 'static> NativeDirectory<H> {
    pub fn new(
        service_id: String,
...
impl<H: AppHost + 'static> NativeService for NativeDirectory<H> {
