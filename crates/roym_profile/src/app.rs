...
pub async fn invoke<H: AppHost>(_host: &H, req: Request) -> Response {
    match req.method.as_str() {
        // This service declares no product verbs, and reports no caller
        // identity: a sibling has no sound way to learn who is asking, so
        // nothing here pretends to. `ping` exists only so the shared suite
        // can prove, on both builds, that a request routed through `web`
        // reaches this service and a real answer comes back --
