...
//! Native service wiring for transaction service.
...
pub struct NativeTransaction<H: AppHost + 'static> {
    service_id: String,
    host_for: Box<dyn Fn(CallerContext) -> H + Send + Sync>,
}

impl<H: AppHost + 'static> fmt::Debug for NativeTransaction<H> {
...
impl<H: AppHost + 'static> NativeTransaction<H> {
    pub fn new(
        service_id: String,
...
impl<H: AppHost + 'static> NativeService for NativeTransaction<H> {
