...
impl ControlPlaneService {
    ...
    async fn status_impl(
        ...
        // Node facts (D-A4-18): gated on node-wide authority, not on seeing
        // any one service -- what this node can run and where it publishes
        // is a property of the node, not of a service grant.
        let node = if self.has_node_wide_ability(caller, Ability::ORCHESTRATOR_STATUS) {
            let (registry_url, dht_enabled) = match self.endpoint_publisher.get() {
                ...
                // A substrate with no publisher wired (a test harness, or a
                // node with no registry role) reports "unknown", not "none"
                // -- a caller must not read an unwired publisher as a
                // split-registry fleet.
                None => (None, false),
            };
            Some(NodeFacts {
        ...
        let mut services = Vec::with_capacity(targets.len() + named_missing.len());
        for service_id in &targets {
            let dep = visible_by_id[service_id.as_str()];
            ...
            // D-A4-7: phase does NOT gate the probe. A `tcp` service is
            // always `Unknown` -- probing only `Running` would mean a
            // declared probe never runs for exactly the type that has no
            // other signal. It is skipped only where the instance is
            // already known to be down, where it would report a second
            // symptom of one fault.
        ...
        // A named id that is not visible: `unauthorized` if it exists on
        // this node but the caller may not see it, `not-found` otherwise.
        // Both are already inferable from `readyz`'s existing error text, so
        // this leaks nothing new -- and the distinction is what lets a
        // poller tell a credential problem from a deploy that never landed.
        for service_id in named_missing {
            let exists = !self.registry.lookup_by_service(&service_id).is_empty();
            services.push(ServiceStatus {
                service_id,
                service_type: None,
                ...
                service_name: None,
                phase: if exists { InstancePhase::Unauthorized } else { InstancePhase::NotFound },
        ...
        match check {
            ...
            WitHealthCheck::HttpGet(p) => {
                ...
                let client = match reqwest::Client::builder()
                    .timeout(Duration::from_millis(u64::from(p.timeout_ms)))
                    .build()
                {
                    Ok(c) => c,
                    Err(e) => {
                        return ProbeStatus::Failing(format!("http client build failed: {e}"));
                    }
                };
                match client.get(&url).send().await {
                    Ok(resp) if resp.status().as_u16() == p.expect_status => ProbeStatus::Passing,
                    Ok(resp) => ProbeStatus::Failing(format!(
                        "expected status {}, got {}",
                        p.expect_status,
                        resp.status().as_u16()
                    )),
                    Err(e) => ProbeStatus::Failing(format!("http probe failed: {e}")),
                }
            }
...
"#;
    ...
    #[tokio::test]
    async fn status_omits_a_service_the_caller_may_not_see_and_reports_unauthorized_when_named() {
        let temp_dir = tempfile::tempdir().unwrap();
        let service = service_for_inline_tests(temp_dir.path()).await;
        ...
        assert!(matches!(named.services[0].phase, InstancePhase::Unauthorized));
    }
