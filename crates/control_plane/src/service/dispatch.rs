...
impl NativeService for ControlPlaneService {
    async fn dispatch(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
        ...
        if invocation.interface.as_str() == SECURITY_INTERFACE {
            return self.dispatch_security_interface(invocation).await;
        }

        if invocation.interface.as_str() == "signing" {
            return self.dispatch_signing_interface(invocation).await;
        }
        ...
        match invocation.method.as_str() {
            "readyz" => self.dispatch_readyz(invocation).await,
            "resolve-instance-identity" => {
                self.dispatch_resolve_instance_identity(invocation).await
            }
            "deploy" => self.dispatch_deploy(invocation).await,
            "write-bindings" => self.dispatch_write_bindings(invocation).await,
            "deploy-plan" => self.dispatch_deploy_plan(invocation).await,
            "undeploy" => self.dispatch_undeploy(invocation).await,
            "restart" => self.dispatch_restart(invocation).await,
            "run-scheduled" => self.dispatch_run_scheduled(invocation).await,
            "renew-cert" => self.dispatch_renew_cert(invocation).await,
            "app-instance-management-of" => {
                self.dispatch_app_instance_management_of(invocation).await
            }
            "claim-app-instance" => self.dispatch_claim_app_instance(invocation).await,
            "release-app-instance" => self.dispatch_release_app_instance(invocation).await,
            "proxy-outbox" => self.dispatch_proxy_outbox(invocation).await,
            "proxy-dead-letters" => self.dispatch_proxy_dead_letters(invocation).await,
            "proxy-replay" => self.dispatch_proxy_replay(invocation).await,
            "sagas" => self.dispatch_sagas(invocation).await,
            "saga-compensate" => self.dispatch_saga_compensate(invocation).await,
            "list" => self.dispatch_list(invocation).await,
            "status" => self.dispatch_status(invocation).await,
            "node-facts-only" => self.dispatch_node_facts_only(invocation).await,
            "republish" => self.dispatch_republish(invocation).await,
...
impl ControlPlaneService {
    /// KEK injection/rotation and vault writes are node-owner operations: a
    /// KEK unlocks every service database on this node, so there is no
    ...
    /// to this interface.
    async fn dispatch_security_interface(
        &self,
        ...
        if !self.has_node_wide_ability(&invocation.caller, Ability::SUBSTRATE_ADMIN) {
            return Err(RpcError::Custom(
                ...
                format!(
                    "caller {} holds no substrate/admin on this substrate; the security interface \
                     is node-owner only",
            ...
            "inject-kek" => {
                let (kek_hex,): (String,) =
                    serde_json::from_value(invocation.params).map_err(|e| {
                        RpcError::InvalidParams(format!("Failed to parse inject-kek params: {e}"))
                    })?;
                ...
                Ok(NativeResponse { payload: serde_json::json!({"status": "injected"}) })
            }
            "rotate-kek" => {
                let (new_kek_hex,): (String,) =
                    serde_json::from_value(invocation.params).map_err(|e| {
                        RpcError::InvalidParams(format!("Failed to parse rotate-kek params: {e}"))
                    })?;
                ...
                self.storage_provider
                    ...
                    .await
                    .map_err(|e| RpcError::InternalError(e.to_string()))?;
                Ok(NativeResponse { payload: serde_json::json!({"status": "rotated"}) })
            }
            "set-secret" => {
                let (service_id, key, value): (String, String, Vec<u8>) =
                    serde_json::from_value(invocation.params).map_err(|e| {
                        RpcError::InvalidParams(format!("Failed to parse set-secret params: {e}"))
                    })?;
                let store = self
                    .storage_provider
                ...
                store
                    ...
                    .await
                    .map_err(|e| RpcError::InternalError(e.to_string()))?;
                Ok(NativeResponse { payload: serde_json::json!({"status": "secret_set"}) })
            }
            _ => Err(RpcError::MethodNotFound(invocation.method)),
        }
    }

    async fn dispatch_signing_interface(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        ...
        match invocation.method.as_str() {
            "identity" => {
                ...
                {
                    s
                } else if let Ok(s) = serde_json::from_value::<String>(invocation.params.clone()) {
                    s
                } else {
                    return Err(RpcError::InvalidParams(
    ...
    async fn dispatch_readyz(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
        let service_id = serde_json::from_value::<(String,)>(invocation.params.clone())
            ...
            .or_else(|_| {
                ...
                serde_json::from_value::<ReadyzPayload>(invocation.params).map(|p| p.service_id)
            })
            .unwrap_or_default();
        self.readyz(service_id, &invocation.caller).await.map_err(RpcError::InternalError)?;
        Ok(ready_response())
    }

    async fn dispatch_resolve_instance_identity(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        ...
        Ok(NativeResponse { payload: serde_json::to_value(identity).unwrap_or(Value::Null) })
    }

    async fn dispatch_deploy(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
    ...
    async fn dispatch_write_bindings(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        let (write,): (BindingWrite,) = serde_json::from_value(invocation.params).map_err(|e| {
            RpcError::InvalidParams(format!("Failed to parse write-bindings params: {e}"))
        })?;
        let outcomes = self
            .write_bindings(write, &invocation.caller)
            .await
            .map_err(RpcError::InternalError)?;
        Ok(NativeResponse { payload: serde_json::to_value(outcomes).unwrap_or(Value::Null) })
    }

    async fn dispatch_deploy_plan(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        let (plan,): (DeploymentPlan,) = serde_json::from_value(invocation.params.clone())
            .or_else(|_| serde_json::from_value::<DeploymentPlan>(invocation.params).map(|p| (p,)))
            .map_err(|e| {
                RpcError::InvalidParams(format!("Failed to parse deploy-plan params: {e}"))
            })?;
        self.deploy_plan(plan, &invocation.caller).await.map_err(RpcError::InternalError)?;
        Ok(NativeResponse { payload: serde_json::json!({"status": "deployed_plan"}) })
    }

    async fn dispatch_undeploy(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
        let (service_id, generation): (String, u64) = serde_json::from_value(invocation.params)
            .map_err(|e| {
    ...
    async fn dispatch_restart(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
        let (service_id, generation): (String, u64) = serde_json::from_value(invocation.params)
            .map_err(|e| RpcError::InvalidParams(format!("Failed to parse restart params: {e}")))?;
    ...
    async fn dispatch_run_scheduled(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        #[derive(serde::Deserialize)]
        ...
        let params = serde_json::from_value::<(String, u64, String, String, Option<String>)>(
            invocation.params.clone(),
        )
        .map(|(service_id, generation, interface, method, params_json)| RunScheduledParams {
            service_id,
            generation,
            interface,
            method,
    ...
    async fn dispatch_renew_cert(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
    ...
    async fn dispatch_app_instance_management_of(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        ...
        let management = self
            ...
            .await
            .map_err(RpcError::InternalError)?;
        Ok(NativeResponse { payload: serde_json::to_value(management).unwrap_or(Value::Null) })
    }

    async fn dispatch_claim_app_instance(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        let (app_instance_id, generation): (String, u64) =
            serde_json::from_value(invocation.params).map_err(|e| {
                RpcError::InvalidParams(format!("Failed to parse claim-app-instance params: {e}"))
            })?;
    ...
    async fn dispatch_release_app_instance(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        let (app_instance_id, generation): (String, u64) =
            serde_json::from_value(invocation.params).map_err(|e| {
                RpcError::InvalidParams(format!("Failed to parse release-app-instance params: {e}"))
            })?;
    ...
    async fn dispatch_proxy_outbox(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
    ...
    async fn dispatch_proxy_dead_letters(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        let service_id = parse_service_id_param(invocation.params);
    ...
    async fn dispatch_proxy_replay(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        #[derive(serde::Deserialize)]
    ...
    async fn dispatch_sagas(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
        let service_id = parse_service_id_param(invocation.params);
        let items =
            self.sagas(service_id, &invocation.caller).await.map_err(RpcError::InternalError)?;
        Ok(NativeResponse { payload: serde_json::to_value(items).unwrap_or(Value::Null) })
    }

    async fn dispatch_saga_compensate(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
        #[derive(serde::Deserialize)]
    ...
    async fn dispatch_list(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
        let services = self.list(&invocation.caller).await.map_err(RpcError::InternalError)?;
        Ok(NativeResponse { payload: serde_json::to_value(services).unwrap_or(Value::Null) })
    }

    async fn dispatch_status(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
        let service_ids = parse_status_params(invocation.params);
        let status =
            self.status(service_ids, &invocation.caller).await.map_err(RpcError::InternalError)?;
        Ok(NativeResponse { payload: serde_json::to_value(status).unwrap_or(Value::Null) })
    }

    /// `status`'s `node` field alone, with none of `status`'s per-service
    /// phase-check-and-probe cost -- for a caller (e.g. `app deploy`'s
    /// preflight) that wants only these four fields.
    async fn dispatch_node_facts_only(
        &self,
        invocation: NativeInvocation,
    ) -> RpcResult<NativeResponse> {
    ...
    /// Node-wide, like `status`: this republishes this node's own endpoint
    /// record, not a single service's, so there is no narrower resource to
    /// scope the gate to.
    async fn dispatch_republish(&self, invocation: NativeInvocation) -> RpcResult<NativeResponse> {
