...
impl SynSvcNativeService {
    ...
    /// Dispatches a `data-layer` native-capability call to the open service
    /// store, in the same table-of-contents shape `blob.rs`'s
    /// `dispatch_blob_store` uses -- one small named method per verb.
    pub(super) async fn dispatch_data_layer(
        ...
        match invocation.method.as_str() {
            "create-collection" | "create_collection" => {
                self.data_create_collection(invocation, store.as_ref()).await
            }
            "drop-collection" | "drop_collection" => {
                self.data_drop_collection(invocation, store.as_ref()).await
            }
            "put" => self.data_put(invocation, store.as_ref()).await,
            "patch" => self.data_patch(invocation, store.as_ref()).await,
            "get" => self.data_get(invocation, store.as_ref()).await,
            "query" => self.data_query(invocation, store.as_ref()).await,
            "delete" => self.data_delete(invocation, store.as_ref()).await,
            "delete-many" | "delete_many" => {
                self.data_delete_many(invocation, store.as_ref()).await
            }
            ...
            "query-raw" | "query_raw" => self.data_query_raw(invocation, store.as_ref()).await,
            "aggregate" => self.data_aggregate(invocation, store.as_ref()).await,
    ...
    /// The admin-capability gate `drop-collection`/`execute-ddl`/
    /// `query-raw` all require, byte-identical across the three: each
    /// bypasses or bends the ordinary per-row policy in a way an
    /// ordinary write/read capability must not reach (see each call
    /// site's own comment for why that verb specifically needs it).
    fn require_data_layer_admin(&self, invocation: &NativeInvocation) -> RpcResult<()> {
    ...
    async fn data_create_collection(
        &self,
        invocation: NativeInvocation,
        store: &dyn ServiceStore,
    ) -> RpcResult<NativeResponse> {
        // Hand-rolled DTO: the bindgen-generated `IndexDefinition` escapes
        // the WIT `type` field as `type_` (a reserved keyword), which
        // doesn't match the plain `type` a JSON caller would naturally
        // send.
    ...
    /// Dropping an *existing* collection bypasses any per-row policy on it
    /// entirely (every row a write-capable-but-otherwise-unreachable
    /// caller could not delete individually goes with it), so it must not
    /// be reachable through an ordinary write capability -- hence the
    /// admin gate. `create-collection` stays ungated deliberately: a
    /// never-yet-existing collection has no policy-protected rows to
    /// destroy, and an owner-rooted UCAN chain can never carry `data-
    /// layer/admin` at all (`router/src/route_handler/io.rs`'s `is_root`
    /// excludes admin-entailing capabilities from per-service owner-
    /// rooting on purpose) -- gating creation too would make it impossible
    /// for a service on an unowned substrate to provision its own schema
    /// at all.
    async fn data_drop_collection(
        &self,
        invocation: NativeInvocation,
        store: &dyn ServiceStore,
    ) -> RpcResult<NativeResponse> {
        self.require_data_layer_admin(&invocation)?;
        #[derive(serde::Deserialize)]
        struct Req {
            name: String,
        }
    ...
    async fn data_get(
        ...
        let auth = self
            ...
            .map_err(data_layer_error)?;
        let outcome =
            store.get(&req.collection, &req.id, auth.as_ref()).await.map_err(data_layer_error)?;
                    ...
                    Some(sieve) if !sieve.abac_permissions.is_empty() => {
                        ...
                        match kept.into_iter().next() {
                            Some((_, extra)) => {
                                ...
                                Some(strip_record(record, &masked).map_err(data_layer_error)?)
                            }
                            None => None,
                        }
                    }
                    _ => Some(
                        strip_record(record, &outcome.masked_fields).map_err(data_layer_error)?,
                    ),
                }
            }
        };
        ...
        let kept: Vec<(RecordReadValue, Vec<String>)> = match sieve {
            Some(sieve) if !sieve.abac_permissions.is_empty() => {
                ...
                .await
                {
                    ...
                    // and denied every row.
                    Err(e) => return Err(data_layer_error(abac_error_to_data_layer_error(e))),
                }
            }
    ...
    async fn data_delete(
        ...
        // `PermissionDenied` (and an FDAE write denial) must surface as a
        // permission denial, not an opaque internal error (pre-existing
        // bug, fixed in passing).
        store.delete(&req.collection, &req.id, auth.as_ref()).await.map_err(data_layer_error)?;
        to_payload(&())
    }

    async fn data_delete_many(
        &self,
        invocation: NativeInvocation,
        store: &dyn ServiceStore,
    ) -> RpcResult<NativeResponse> {
        ...
        let auth = self
            ...
            .await
            .map_err(data_layer_error)?;
        // `data_layer_error`, not `internal` -- `delete_many`'s stage-4
        // `PermissionDenied` (`sqlite.rs`) was already surfacing as an
        // opaque internal error rather than a permission denial before
        // this fix (pre-existing bug, fixed in passing, same class as
        // `delete`/`batch-mutate`).
        let affected = store
    ...
    async fn data_batch_mutate(
        &self,
        invocation: NativeInvocation,
        store: &dyn ServiceStore,
    ) -> RpcResult<NativeResponse> {
        // Hand-rolled DTO: the bindgen-generated `Mutation` variant derives
        // serde's default externally-tagged representation (e.g. `{"Put":
        // {...}}`, PascalCase), which doesn't match this API's snake_case
        // JSON convention.
        #[derive(serde::Deserialize)]
        #[serde(tag = "type", content = "value", rename_all = "snake_case")]
        enum MutationDto {
        ...
        let auth = self
            ...
            .await
            .map_err(data_layer_error)?;
        // `data_layer_error`, not `internal` -- same fix as `delete` above
    ...
    /// `is_init_context` scaffold): only a caller holding `data-layer/
    /// admin` on this service's resource may run DDL. Lifecycle init/
    /// migrate runs as `AuthLevel::LocalElevated` (`CallerContext::
    /// local_elevated`), which carries it; an ordinary caller does not.
    async fn data_execute_ddl(
        &self,
        invocation: NativeInvocation,
        store: &dyn ServiceStore,
    ) -> RpcResult<NativeResponse> {
    ...
    /// Admin-capability gate -- identical reasoning to `data_execute_ddl`.
    async fn data_query_raw(
        &self,
        invocation: NativeInvocation,
        store: &dyn ServiceStore,
    ) -> RpcResult<NativeResponse> {
    ...
    /// No capability gate -- unlike `data_execute_ddl`/`data_query_raw`,
    /// `aggregate` compiles a whitelisted operator document, the same
    /// trust level as `data_query`.
    async fn data_aggregate(
        &self,
        invocation: NativeInvocation,
        store: &dyn ServiceStore,
    ) -> RpcResult<NativeResponse> {
