...
//! Native service wiring for catalog service.
...
pub struct NativeCatalog<H: AppHost + 'static> {
    service_id: String,
    host_for: Box<dyn Fn(CallerContext) -> H + Send + Sync>,
}

impl<H: AppHost + 'static> fmt::Debug for NativeCatalog<H> {
...
impl<H: AppHost + 'static> NativeCatalog<H> {
    pub fn new(
        service_id: String,
...
impl<H: AppHost + 'static> NativeService for NativeCatalog<H> {
