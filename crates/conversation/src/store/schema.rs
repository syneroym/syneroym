...
impl ConversationStore {
    ...
    /// exist, running `TABLE_GROUP_DDL` in order — a table with a
    /// `REFERENCES` clause is declared after the table it points at.
    fn init_schema(conn: &Connection) -> Result<()> {
        for ddl in TABLE_GROUP_DDL {
...
/// One DDL script per table group, run in order by `init_schema`. Split
/// from one giant script purely to keep each piece readable — every
/// statement is `IF NOT EXISTS`, so the grouping itself has no effect on
/// the resulting schema, only the relative order across groups does (a
/// `REFERENCES` target must already exist).
const TABLE_GROUP_DDL: &[&str] = &[
    ...
    // sync_cursors, message_recipients
