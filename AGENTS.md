...
- **Function length.** Hard limit of 100 lines for any function. Proactively decompose functions during implementation. If your function trips `clippy::too_many_lines`, extract into helper functions; do not raise the threshold.
...
- **Look for an existing helper before writing a new one.** Duplication here is
  ...
  *Checked by:* `cargo xtask check-duplication` (and `mise run
  check:duplication`), ratcheted at `--max-exact-percent 9.1` for now.
