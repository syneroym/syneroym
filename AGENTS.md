...
- **Function length.** Hard limit of 100 lines for any function. Proactively decompose functions during implementation. If your function trips `clippy::too_many_lines`, extract into helper functions; do not raise the threshold. Test functions may exceed 100 lines only with a per-function `#[allow(clippy::too_many_lines)]`, never a file-level one, and total workspace suppressions are capped by a ratchet.
  *Checked by:* `cargo clippy --workspace --all-targets --all-features` (enforced via `clippy.toml` `too-many-lines-threshold = 100`) and `cargo xtask check-lint-suppressions`.
...
- **File size.** Production source files must not exceed 800 lines; prefer new sibling files over expanding existing ones. Test files must not exceed 1,800 lines (a ratchet ceiling moving toward 800). When splitting an oversized file `foo.rs`, keep `foo.rs` as the module entry and place submodules in `foo/` (e.g., `foo/bar.rs`). Test modules over ~500 lines move to a sibling file under `tests/`.
