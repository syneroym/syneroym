...
- **`syneroym-roym-core`** + **`syneroym-roym-{web,profile,conversation,catalog,transaction,directory}`** — the Roym product: a SynApp of six services (`crates/roym_core/app/roym.toml`) sharing one JSON-RPC-shaped envelope (`syneroym-roym-core::envelope`) over a single `invoke`/`status` WIT interface each. `web` is the only inbound HTTP surface (the Hub UI, `crates/roym_web/ui`); `conversation` and `catalog` also declare a `depends_on` edge to `profile`. Every service admits an `api.invoke` only from a local dispatch path (`roym_core::admit::require_internal`, over `syneroym:invocation`) — a call arriving over the wire is refused — so the person's browser through `web` is the one legitimate ingress; `api.status` is the one export left open, for the health probe. Built both as `wasm32-wasip2` components and, behind `syneroym-substrate`'s `roym` Cargo feature, linked in natively through the dual-build shim (`syneroym-app-host`/`syneroym-app-host-native`) — one source tree, either build.
