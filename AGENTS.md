...
- **`syneroym-wit-interfaces`** — generates Rust host/guest types from the WIT interfaces under `crates/wit_interfaces/wit/` (`host`, `data-layer`, `blob-store`, `app-config`, `control-plane`, `vault`, `signing`, `http`, `conversation`) via `wit-bindgen`/`bindgen!`; this is the WASM component boundary — host-side crates (e.g. `syneroym-data-db`, `syneroym-data-blob`) speak these generated types directly with no separate conversion layer.
...
- **`syneroym-roym-core`** + **`syneroym-roym-{web,profile,conversation,catalog,transaction,directory}`** — the Roym product: a SynApp of six services (`crates/roym_core/app/roym.toml`) sharing one JSON-RPC-shaped envelope (`syneroym-roym-core::envelope`) over a single `invoke`/`status` WIT interface each. `web` is the only inbound HTTP surface (the Hub UI, `crates/roym_web/ui`) and the only one with `depends_on`; every other service is reachable only as a bound dependency or through the gateway hostname scheme. Built both as `wasm32-wasip2` components and, behind `syneroym-substrate`'s `roym` Cargo feature, linked in natively through the dual-build shim (`syneroym-app-host`/`syneroym-app-host-native`) — one source tree, either build.
