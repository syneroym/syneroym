...
- **`syneroym-app-orchestration`** + **`syneroym-sandbox-app`** — the "orchestrator" native service: manages the deployed-app catalog/lifecycle (`catalog.rs`, `compiler.rs`, `reconcile.rs`, `resolver.rs`) and runs user WASM components (via Wasmtime) or delegates to `syneroym-sandbox-podman` for OCI container services.
